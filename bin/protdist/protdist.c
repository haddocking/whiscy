...
//Modified November 2003 by Sjoerd de Vries. MOD indicates modifications
...
inline void predict(long nb1, long nb2) //MOD
{
...
#endif
  if (argc < 3) { //MOD
    printf("Too few arguments for this modified version of PROTDIST\n"); //MOD
    printf("Usage: protdist <infile> <outfile>\n"); //MOD
    exit(1); //MOD
  } //MOD

  openfile(&infile,argv[1],"r",argv[0],infilename); //MOD
  openfile(&outfile,argv[2],"w",argv[0],outfilename); //MOD
